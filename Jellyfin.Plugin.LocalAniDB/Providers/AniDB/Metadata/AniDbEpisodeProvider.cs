...
namespace Jellyfin.Plugin.LocalAniDB.Providers.AniDB.Metadata
{
    ...
    public class AniDbEpisodeProvider : IRemoteMetadataProvider<Episode, EpisodeInfo>
    {
        ...
        /// Only calls GetSeriesData (which downloads from the API) if the folder doesn't exist yet.
        /// </summary>
        private async Task<string> FindSeriesFolder(string seriesId, CancellationToken cancellationToken)
        {
            ...
            // If the folder already has a series.xml, skip the expensive download cycle
